<template>
  <div>
    <!--    <h2>多选题浏览编辑功能</h2>-->
    <h3>[多选] {{ message.title }}&nbsp&nbsp&nbsp<i class="el-icon-key"></i></h3>
    ...
    <el-input placeholder="增加选项内容" style="width: 300px" v-model="text3"
              v-show="message.true_xuanxiang.length <= 10"></el-input>
    <el-input placeholder="选项数量已超过上限" style="width: 300px" v-model="text3" v-show="message.true_xuanxiang.length > 10"
              disabled></el-input>
    &nbsp
    <el-button @click="add" v-show="message.true_xuanxiang.length <= 10">增加一个选项</el-button>
    <el-button @click="add" v-show="message.true_xuanxiang.length > 10" disabled>增加一个选项</el-button>
    <el-button type="primary" @click="delete1" v-show="status === true & message.true_xuanxiang.length >= 2">删除最后一个选项
    </el-button>
    <el-button type="primary" @click="delete1" v-show="status === true & message.true_xuanxiang.length <= 1" disabled>
      删除最后一个选项
    </el-button>
    <el-button type="primary" @click="delete2" v-show="status === false & message.true_xuanxiang.length >= 2">删除第一个选项
    </el-button>
    <el-button type="primary" @click="delete2" v-show="status === false & message.true_xuanxiang.length <= 1" disabled>
      删除第一个选项
    </el-button>
