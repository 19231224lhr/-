<template>
  <div>
    <strong style="font-size: 20px" v-show="message.score !== 0">[单选] {{ message.title }}
      ({{ message.score }})</strong>
    <strong style="font-size: 20px" v-show="message.score === 0">[单选] {{ message.title }}</strong>
    ...
    <el-button type="primary" @click="delete2" v-show="status === false & message.true_xuanxiang.split(';').length >= 2">
      删除第一个选项
    </el-button>
